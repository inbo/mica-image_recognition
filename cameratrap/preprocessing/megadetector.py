# derived from https://github.com/microsoft/CameraTraps/blob/master/detection/run_tf_detector.py
...
def truncate_float(x, precision=3):
    ...
    Extracted from https://github.com/microsoft/CameraTraps/blob/master/ct_utils.py
    """
...
class TFDetector:
    ...
    def __convert_coords(self, image, tf_coords):
        """Converts coordinates from the model's output format [y0, x0, y1, x1] to the
        format used by pillow package [x0, y0, x1, y1]. Model outputs coordinates
        are normalized in the range [0, 1].

        Parameters
        ----------
        tf_coords: np.array
            Predicted bounding box coordinates from the TF detector,
            has format [y0, x0, y1, x1]
        Returns
        -------
        list
            Predicted bounding box coordinates [x0, y0, x1, y1]
        """
        width, height = image.size
        return [round(tf_coords[1]*width),
                round(tf_coords[0]*height),
                round(tf_coords[3]*width),
                round(tf_coords[2]*height)
               ]
    ...
    def generate_detections_one_image(self, image, image_id,
        ...
        try:
            ...
            for b, s, c in zip(boxes, scores, classes):
                if s > detection_threshold:
                    detection_entry = {
                        ...
                        'bbox': self.__convert_coords(image=image, tf_coords=b)
                    }
            ...
            result['failure'] = self.FAILURE_TF_INFER
