...
########################################################################
def top_predictions(prediction_probabilities, data_hierarchy):
    """Determine the prediction of the sequences based on the top-k predictions at every level.
    """

    #sequences = data_hierarchy['sequence'].drop_duplicates()
    data_seq_top = pd.DataFrame(
        columns=['level_1','level_1_p','level_2','level_2_p',
                 'level_3','level_3_p','level_4','level_4_p','level_5','level_5_p'])

    #for s, seq in enumerate(sequences):
    data_sequence = data_hierarchy # data_hierarchy[data_hierarchy['sequence'] == seq]
    pred_sequence = prediction_probabilities # data.loc[:,'0':'16'].loc[data['sequence'] == seq]
    ...
    data_seq_top.loc[0] = [l1, p_l1, l2, p_l2, l3, p_l3, l4, p_l4, l5, p_l5]

    return data_seq_top

#########################################################################################
def hierarchical_predictions(predictions):
    """
    This function determines the hierarchical prediction for the individual images,
    based on the output of the neural network. These predictions can then be used to classify a sequence.
    """
...
def bottom_hierarchical_prediction(x):
    """
    This function determines the final prediction for a sequence,
    based on the hierarchical prediction at every level.
    """
    ...
    labels = ['Ass','Beech Marten','Bird','Blank','Cat','Squirrel','Hare',
              'Horse','Human','Mouse','PickupSetup','Fox','Dog','Mouflon',
              'Hedgehog','Roe Deer','Wild Boar']
    label = labels[label]
